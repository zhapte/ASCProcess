...
public sealed class CollisionDownloadView : View
{
    ...
    public CollisionDownloadView(IApplication application, string envPath)
    {
    ...
    {
        if (_cancellation is not null) return;
        string claim = _claim.Text.Trim();
        if (string.IsNullOrWhiteSpace(claim))
        { Log("Claim number is required."); _claim.SetFocus(); return; }
        _claim.Text = claim;
        _log.Text = string.Empty;
        Log($"Starting download for {claim}.");
        _claim.Enabled = _start.Enabled = false;
        _stop.Enabled = true;
        ...
        try
        {
            await Task.Run(() => new DownloadWorkflow().RunAsync(
                claim, _envPath, Log, cancellation.Token));
        ...
        finally
        {
            ...
            {
                if (_disposed) return;
                _claim.Enabled = _start.Enabled = true;
                _stop.Enabled = false;
                _reset.Enabled = true;
            });
        }
